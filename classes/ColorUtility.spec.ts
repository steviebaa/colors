...
describe('ColorUtility.rgb', () => {
	...
	describe('.to', () => {
		...
		it('should convert an RGB color to HSL', () => {
			...
			expect(hsl('rgb(49,94,141,0.4)')).toEqual(
				'hsl(211deg,48%,37%,0.4)'
			);
			expect(hsl('rgb(49,94,141/0.4)')).toEqual(
				'hsl(211deg,48%,37%,0.4)'
			);
			...
			expect(hsl('rgb(49,94  141 / 0.0)')).toEqual(
				'hsl(211deg,48%,37%,0)'
			);
...
describe('ColorUtility.hsl', () => {
	...
	describe('.set', () => {
		it('should set the hue of an HSL color', () => {
			...
			expect(set.hue('hsl(8deg,8%,7%,0.4)', 3)).toEqual(
				'hsl(3deg,8%,7%,0.4)'
			);
			expect(set.hue('hsl(8deg,8%,7%)', -11)).toEqual(
				'hsl(349deg,8%,7%)'
			);
		...
		it('should set the saturation of an HSL color', () => {
			...
			expect(s('hsl(8deg,8%,7%,0.4)', 101)).toEqual(
				'hsl(8deg,100%,7%,0.4)'
			);
		});
		...
		it('should set the opacity of an HSL color', () => {
			...
			expect(opacity('hsl(8deg,8%,7%)', 2)).toEqual(
				'hsl(8deg,8%,7%,0.02)'
			);
			expect(opacity('hsl(8deg,8%,7%,0.4)', 10)).toEqual(
				'hsl(8deg,8%,7%,0.1)'
			);
		});
	});
	it('should decompose an HSL color to numeric values', () => {
		...
		expect(decompose('hsl(211deg,48%,37%,0.4)')).toEqual([
			211, 48, 37, 0.4,
		]);
		expect(decompose('hsl(211deg,48%,37% / 5)')).toEqual([
			211, 48, 37, 0.05,
		]);
		expect(decompose('hsl(211deg,48%,37%, 0.5)')).toEqual([
			211, 48, 37, 0.5,
		]);
	});
	it('should decompose an HSL color to string values', () => {
		...
		expect(d('hsl(21deg,9%,8% / 5)')).toEqual([
			'21deg',
			'9%',
			'8%',
			'0.05',
		]);
		expect(d('hsl(21deg,9%,8%, 0.5)')).toEqual([
			'21deg',
			'9%',
			'8%',
			'0.5',
	...
	it('should compose an HSL color to string values', () => {
		...
		expect(r(['21deg', '9%', '8%', '0.05'])).toEqual(
			'hsl(21deg,9%,8%,0.05)'
		);
	...
	it('should clean an HSL color to a predictable format', () => {
		...
		expect(clean('hsl(21deg,48%,37%,0.4)')).toEqual(
			'hsl(21deg,48%,37%,0.4)'
		);
		expect(clean('hsl(21deg,48%,37% / 5)')).toEqual(
			'hsl(21deg,48%,37%,0.05)'
		);
		expect(clean('hsl(21deg,48%,37%, 0.5)')).toEqual(
			'hsl(21deg,48%,37%,0.5)'
		);
		expect(clean('hsl(21deg,48%,37%, )')).toEqual('hsl(21deg,48%,37%)');
	});
	describe('.to', () => {
		...
		it('should convert an HSL color to HSLA with alpha', () => {
			const { to } = ColorUtility.hsl;
			expect(to.hsla('hsl(2deg,8%,9%)', 0.5)).toEqual(
				'hsl(2deg,8%,9%,0.5)'
			);
...
describe('ColorUtility', () => {
	describe('ColorUtility.get ', () => {
		...
		it('should get the opacity of a supported color', () => {
			...
			expect(() => get.opacity('bad')).toThrow(
	...
	describe('ColorUtility.to ', () => {
		...
		it('should convert a supported color to an HSL color', () => {
			...
			expect(to.hsl('hsl(2deg,8%,9% / 5)')).toEqual(
				'hsl(2deg,8%,9%,0.05)'
			);
